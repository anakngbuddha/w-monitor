...
func main() {
	...
	if controls := serviceControlCount(); controls > 1 {
		log.Fatal("choose exactly one service-control action")
	}
	if *flagStop || *flagStart || *flagUninstall {
		controlService()
		return
	}
	if err := loadConfigEnv(); err != nil {
		log.Fatal("configuration rejected: ", err)
	}
	applyEnvConfig()
	alertEnvDefaults()
	...
	if *flagShowKey {
		fmt.Println("Secret recovery output is disabled. Request a new scoped credential from your administrator.")
		return
	}
	if *flagPrintConfig {
		printConfig()
	...
	if err != nil {
		prg.Stop(nil)
		log.Fatal("service setup failed")
	}
	...
	if err := runForeground(prg); err != nil {
		log.Fatal(err)
	}
}
...
func serviceControlCount() int {
	count := 0
	for _, value := range []bool{*flagInstall, *flagUninstall, *flagStart, *flagStop} {
		if value {
			count++
		}
	}
...
func controlService() {
	...
	if *flagInstall {
		for name := range explicitFlags {
			if _, secret := secretServiceFlags["-"+name]; secret {
				log.Fatal("put service secrets in protected machine config, not installation arguments")
			}
		}
	...
	if err != nil {
		log.Fatal("service-control operation failed: ", err)
	}
