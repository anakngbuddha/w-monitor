...
func browserAssets() (string, string, error) {
	...
	// Normalize embedded source line endings before applying checked multiline
	// anchors so Windows and Unix checkouts enforce the same transformation.
	...
	for _, patch := range patches {
		if strings.Count(js, patch[0]) != 1 {
			return "", "", fmt.Errorf("dashboard: security patch anchor changed: %q", patch[0])
		}
