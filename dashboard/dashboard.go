...
// browserAssets migrates the existing dashboard without changing its layout or
// duplicating the chart/sizing code. The legacy HTML is an embedded source
// template, NEVER a publicly accessible asset. Every expected patch is checked;
// source drift fails registration instead of serving a bearer-token fallback.
// Remove this adapter when the canonical dashboard replaces legacy sizing in P3.
func browserAssets() (string, string, error) {
	...
	if start < 0 || end <= start || strings.Count(html, "<script>") != 1 {
		return "", "", fmt.Errorf("dashboard: unexpected script structure")
	}
	...
	if a < 0 || b <= a {
		return "", "", fmt.Errorf("dashboard: missing legacy authentication boundary")
	}
	js = js[:a] + string(auth) + "\n" + js[b:]
	patches := [][2]string{
	...
	for _, p := range patches {
		if strings.Count(js, p[0]) != 1 {
			return "", "", fmt.Errorf("dashboard: security patch anchor changed")
		}
		js = strings.Replace(js, p[0], p[1], 1)
	}
	js = strings.ReplaceAll(js, "spanGaps: true", "spanGaps: false")
	// Preserve styles and layout, but remove external font requests, inline
	// event handlers and CDN scripts before any HTML can reach a browser.
	html = regexp.MustCompile(`(?m)^\s*<link[^>]+https://fonts\.[^>]+>\s*`).ReplaceAllString(html, "")
	...
	if strings.Contains(js, "getApiKey(") || strings.Contains(js, "localStorage.setItem") || strings.Contains(html, "https://fonts.") || strings.Contains(html, "https://cdn.") {
		return "", "", fmt.Errorf("dashboard: unsafe legacy asset survived migration")
	}
	return html, js, nil
}
...
func Register(srv *server.Server) error {
	...
	// An explicit allowlist prevents /static/index.html, encoded paths or a
	// FileServer fallback from exposing the unmodified source template.
