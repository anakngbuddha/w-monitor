...
func (persistentServerIDFlag) Set(raw string) error {
	if !validServerID.MatchString(raw) {
		return fmt.Errorf("server-id must contain 1-128 ASCII letters, digits, underscores, dots or hyphens")
	}
	explicitServerID = raw
	return nil
}
func init() {
	flag.Var(persistentServerIDFlag{}, "server-id", "Stable agent server identity (validated before use)")
}
