...
func (p *program) Start(service.Service) error {
	...
	if p.collector != nil {
		start(func() { p.collector.Run(p.ctx) })
	}
	if p.evaluator != nil {
		start(func() { p.evaluator.Run(p.ctx) })
	}
	if p.retention != nil {
		start(func() { p.retention.RunContext(p.ctx) })
	}
	if p.srv != nil {
		start(func() {
...
func (p *program) Stop(service.Service) error {
	p.stopOnce.Do(func() {
		...
		if p.store != nil {
			if *flagAgentHub == "" && (*flagRunFor > 0 || *flagExportFlt != "") {
				writeShutdownExport(p.store)
			}
...
func runForeground(p *program) error {
	...
	if *flagRunFor > 0 {
		timer = time.NewTimer(*flagRunFor)
		deadline = timer.C
		defer timer.Stop()
	}
	var runErr error
...
func makeProgram() (*program, error) {
	if *flagAgentHub != "" {
		...
		bindAgentSpool(ag)
		if err := ag.InitializationError(); err != nil {
			ag.Close()
			return nil, err
		}
	...
	if ret != nil {
		if budget, ok := positiveEnvInt("WMONITOR_DISK_BUDGET_BYTES"); ok {
			ret.SetDiskBudget(budget)
		}
	}
	...
	if *flagHub {
		...
		if !ok {
			store.Close()
			return nil, errors.New("backend cannot authenticate Hub requests")
		}
	...
	if !*flagHub {
		...
		if ret != nil {
			sink = capacityStore{Store: store, retention: ret}
		}
...
func (s capacityStore) InsertMetric(row storage.MetricRow) error {
	if err := s.retention.CheckCapacity(); err != nil {
		return err
	}
	return s.Store.InsertMetric(row)
}
func (s capacityStore) InsertProcess(row storage.ProcessRow) error {
	if err := s.retention.CheckCapacity(); err != nil {
		return err
	}
	return s.Store.InsertProcess(row)
}
...
func configureUserTracker(col *collector.Collector) {
	...
	if len(ports) == 1 {
		tracker.SetExcludePorts(ports[0])
	}
...
func agentCredentials() (string, error) {
	...
	if creds, err := agent.LoadCredentials(); err == nil && creds != nil {
		if !agent.SameHubOrigin(creds.HubURL, *flagAgentHub) || creds.ServerID != resolveServerID() {
			return "", errors.New("stored credential destination or identity changed; explicit re-enrollment required")
		}
		return creds.Token, nil
	} else if err != nil && !errors.Is(err, agent.ErrNoCredentials) {
		return "", errors.New("stored credentials could not be read safely")
	}
	code := resolveEnrollCode()
	if code == "" {
		return "", errors.New("configure an agent credential or enrollment code in the protected config")
	}
	...
	if err != nil {
		return "", errors.New("enrollment failed; no credential was logged")
	}
	return creds.Token, nil
}
...
func runAgentMode() {
	...
	if err := runForeground(p); err != nil {
		log.Fatal(err)
	}
}
func runAgentAsService(apiKey string) {
	...
	svc, err := service.New(p, &service.Config{Name: "wmonitor"})
	if err != nil {
		log.Fatal(err)
	}
...
func runOneShotExport() bool {
	if *flagExportCSV == "" && *flagExportTxt == "" && *flagAssessmentReport == "" {
		return false
	}
	if *flagHub && strings.TrimSpace(*flagTenant) == "" {
		log.Fatal("Hub export requires an explicit -tenant")
	}
	store, _, err := openStore()
	if err != nil {
		log.Fatal("export storage unavailable")
	}
	defer store.Close()
	start, end := time.Now().Add(-*flagSince), time.Now()
	switch {
	case *flagExportCSV != "":
		_, err = export.CSVReport(store, start, *flagExportCSV, exportTenant())
	case *flagExportTxt != "":
		_, err = export.TextReport(store, start, *flagExportTxt, exportTenant())
	default:
		err = export.GenerateAssessmentReport(store, start, end, *flagAssessmentReport, exportTenant())
	}
	if err != nil {
...
func writeShutdownExport(store storage.Store) {
	...
	if *flagHub && *flagTenant == "" {
		log.Print("[export] skipped: explicit tenant required")
		return
	}
	name := "wmonitor_export_" + time.Now().Format("20060102_150405")
	var err error
	if runtime.GOOS == "windows" {
		_, err = export.CSVReport(store, time.Now().Add(-window), name+".csv", exportTenant())
	} else {
		_, err = export.TextReport(store, time.Now().Add(-window), name+".txt", exportTenant())
	}
	if err != nil {
		log.Print("[export] shutdown export failed")
	}
}
