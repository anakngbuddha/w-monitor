...
func mint(t *testing.T, db *storage.DB, record storage.APIKeyRecord, plaintext string) string {
	t.Helper()
	if record.KeyHash == "" { record.KeyHash = storage.HashAPIKey(plaintext) }
	if record.KeyPrefix == "" { record.KeyPrefix = storage.ExtractKeyPrefix(plaintext) }
	if record.CreatedAt.IsZero() { record.CreatedAt = time.Now() }
	if err := db.UpsertAPIKey(record); err != nil { t.Fatal(err) }
	return plaintext
}

func TestP103RouteMethodKindScopeMatrix(t *testing.T) {
	t.Setenv("WMONITOR_ALLOW_INSECURE_ENROLL", "1")
	db, err := storage.Open(filepath.Join(t.TempDir(), "matrix.db")); if err != nil { t.Fatal(err) }; defer db.Close()
	...
	type row struct { name, method, path, key string; body []byte; want int }
	cases := []row{
	...
	for _, tc := range cases { t.Run(tc.name, func(t *testing.T) { if w := do(srv, tc.method, tc.path, tc.key, tc.body); w.Code != tc.want { t.Fatalf("%s: got %d want %d: %s", tc.path, w.Code, tc.want, w.Body.String()) } }) }
	body, _ := json.Marshal(map[string]string{"enroll_code": code, "server_id": "srv-new"})
	if w := do(srv, "POST", "/api/enroll", "", body); w.Code != http.StatusCreated { t.Fatalf("handshake failed: %d %s", w.Code, w.Body.String()) }
}

func TestP103AgentTokenCannotWriteSiblingServer(t *testing.T) {
	db, err := storage.Open(filepath.Join(t.TempDir(), "bound.db")); if err != nil { t.Fatal(err) }; defer db.Close()
	srv := server.New(db, "0"); srv.EnableHubMode(db)
	...
	for _, attempt := range []struct{ kind string; body []byte }{{"metric", metric}, {"process", process}} {
		if w := do(srv, "POST", "/api/ingest?type="+attempt.kind, token, attempt.body); w.Code != 400 { t.Fatalf("invalid bound identity: %d", w.Code) }
	}
	rows, err := db.QueryMetrics(time.Now().Add(-time.Hour), "t_bound"); if err != nil || len(rows) != 0 { t.Fatal("spoofed metric inserted") }
	processes, err := db.QueryProcesses(time.Now().Add(-time.Hour), "t_bound"); if err != nil || len(processes) != 0 { t.Fatal("spoofed process inserted") }
	metric, _ = json.Marshal(storage.MetricRow{Timestamp: time.Now(), ServerID: "host-a", CPUPct: 2, MemPct: 2})
	if w := do(srv, "POST", "/api/ingest?type=metric", token, metric); w.Code != 202 { t.Fatalf("matching ingest failed: %d", w.Code) }
}

func TestP103RevokedKeyStaysRevokedAfterRestart(t *testing.T) {
	dir := t.TempDir(); path := filepath.Join(dir, "restart.db")
	db, err := storage.Open(path); if err != nil { t.Fatal(err) }
	...
	if err := db.UpsertAPIKey(record); err != nil { t.Fatal(err) }
	if _, err := db.RevokeAPIKey("default"); err != nil { t.Fatal(err) }
	if err := os.WriteFile(filepath.Join(dir, "clients_registry.csv"), []byte("ClientName,APIKey\ndefault,"+key+"\n"), 0600); err != nil { t.Fatal(err) }
	_ = db.UpsertAPIKey(record)
	if err := db.Close(); err != nil { t.Fatal(err) }
	db, err = storage.Open(path); if err != nil { t.Fatal(err) }; defer db.Close()
	srv := server.New(db, "0"); srv.EnableHubMode(db)
	if w := do(srv, "GET", "/api/metrics", key, nil); w.Code != 401 { t.Fatal("revoked credential resurrected") }
}

func TestP103DuplicateClientNamesNeverResolveIdentity(t *testing.T) {
	t.Setenv("WMONITOR_ALLOW_INSECURE_ENROLL", "1")
	db, err := storage.Open(filepath.Join(t.TempDir(), "dup.db")); if err != nil { t.Fatal(err) }; defer db.Close()
	srv := server.New(db, "0"); srv.EnableHubMode(db)
	admin, _ := storage.GenerateToken(storage.KindAdmin)
	mint(t, db, storage.APIKeyRecord{TenantID: "admin", ClientName: "Admin", Kind: storage.KindAdmin, Scope: storage.ScopeAdmin}, admin)
	...
	if w := do(srv, "POST", "/api/admin/enroll-codes", admin, body); w.Code != 409 { t.Fatal("ambiguous name resolved") }
	body, _ = json.Marshal(map[string]string{"client_name": "Acme"})
	if w := do(srv, "POST", "/api/admin/clients", admin, body); w.Code != 409 { t.Fatal("duplicate client created") }
	body, _ = json.Marshal(map[string]interface{}{"client_name": "Acme", "tenant_id": "t_one", "max_uses": 1, "ttl_hours": 1})
	if w := do(srv, "POST", "/api/admin/enroll-codes", admin, body); w.Code != 201 { t.Fatalf("explicit tenant rejected: %d", w.Code) }
}

func TestP103ReadTokenCannotRevokeAgent(t *testing.T) {
	db, err := storage.Open(filepath.Join(t.TempDir(), "norevoke.db")); if err != nil { t.Fatal(err) }; defer db.Close()
	srv := server.New(db, "0"); srv.EnableHubMode(db)
	read, _ := storage.GenerateToken(storage.KindRead); agent, _ := storage.GenerateToken(storage.KindAgent)
	...
	if w := do(srv, "DELETE", "/api/admin/agents?server_id=srv-keep", read, nil); w.Code != 403 { t.Fatal("read-only revocation allowed") }
	if _, err := db.ResolveAPIKey(storage.HashAPIKey(agent)); err != nil { t.Fatal("read-only request changed agent credential") }
}
