...
func TestDailyQuotaIsTenantScopedAndResets(t *testing.T) {
	...
	if !q.allow("a", 1) || !q.allow("a", 1) || q.allow("a", 1) || !q.allow("b", 1) {
		t.Fatal("tenant quota isolation failed")
	}
	now = now.Add(24 * time.Hour)
	if !q.allow("a", 1) {
		t.Fatal("UTC rollover failed")
	}
}
func TestDailyQuotaCountsRowsAtomically(t *testing.T) {
	q := newDailyQuota(3)
	if !q.allow("tenant", 2) || q.allow("tenant", 2) || !q.allow("tenant", 1) {
		t.Fatal("rejected reservation consumed allowance")
	}
}
...
func TestAuthenticationDoesNotDebitAcceptedData(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		if !enforceDailyIngestQuota(httptest.NewRecorder(), r, "tenant") {
			t.Fatal("authentication charged an unvalidated body")
		}
	}
	if !ingestDailyQuota.allow("tenant", 1) {
		t.Fatal("auth consumed accepted-data allowance")
	}
}
...
func TestAcceptedQuotaRejectsNewRowsButAllowsReplayAcrossServerRestart(t *testing.T) {
	...
	if err := db.UpsertAPIKey(storage.APIKeyRecord{KeyHash: storage.HashAPIKey(token), TenantID: "tenant", ClientName: "Fixture", Kind: storage.KindAgent, Scope: storage.ScopeIngest, ServerID: "agent"}); err != nil {
		t.Fatal(err)
	}
	srv := New(db, "0")
	...
	if w := request(srv, []byte(`{"bad":true}`)); w.Code != 400 {
	...
	if w := request(srv, body); w.Code != 202 {
		t.Fatalf("first accepted=%d %s", w.Code, w.Body.String())
	}
	restarted := New(db, "0")
	restarted.ingestPolicy = srv.ingestPolicy
	restarted.EnableHubMode(db)
	if w := request(restarted, body); w.Code != 202 {
		t.Fatalf("replay at exhausted budget=%d", w.Code)
	}
	batch.Events[0].EventID, batch.Events[0].Sequence = "two", 2
	body, _ = json.Marshal(batch)
	if w := request(restarted, body); w.Code != http.StatusTooManyRequests || w.Header().Get("Retry-After") == "" {
		t.Fatalf("new row bypassed durable quota: %d", w.Code)
	}
	rows, err := db.QueryMetricsQ(storage.MetricQuery{Ctx: context.Background(), Since: time.Now().Add(-time.Hour), TenantID: "tenant"})
	if err != nil || len(rows) != 1 {
		t.Fatal("malformed/replayed/rejected requests changed accepted data")
	}
}
