...
func New(db storage.Store, port string) *Server {
	...
	if err := s.ingestPolicy.Validate(); err != nil {
		s.ingestionErr = err
	}
	s.routes()
	return s
...
func (s *Server) EnableHubMode(keys KeyStore) {
	...
	s.mux.HandleFunc("/api/admin/audit", s.handleAdminAudit)
	s.mux.HandleFunc("/api/admin/agents/expected", s.handleExpectedAgents)
	s.syncAuthEpoch()
	if backend, ok := s.db.(ingestBackend); ok && s.ingestionErr == nil {
		ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
		defer cancel()
		if err := backend.InitializeIngest(ctx); err != nil {
			s.ingestionErr = errors.New("ingest initialization failed")
		} else {
			s.ingestion = backend
		}
	} else if s.ingestionErr == nil {
		s.ingestionErr = errors.New("storage lacks atomic ingestion")
	}
	if keys == nil {
		s.ingestionErr = errors.New("authentication unavailable")
	}
}
...
func (s *Server) trackViewer(r *http.Request) {
	...
	defer s.mu.Unlock()
	now := time.Now()
	for ip, seen := range s.dashboardViewers {
		if now.Sub(seen) > time.Minute {
			delete(s.dashboardViewers, ip)
		}
	}
	if len(s.dashboardViewers) < 4096 {
		s.dashboardViewers[clientIP(r)] = now
	}
}
...
func (s *Server) Start() error {
	if s.hubMode && s.ingestionErr != nil {
		return s.ingestionErr
	}
	host := "127.0.0.1"
	if s.hubMode {
		host = "0.0.0.0"
	...
	if configured := os.Getenv("WMONITOR_LISTEN_HOST"); configured != "" {
		...
		if ip == nil || (!s.hubMode && !ip.IsLoopback()) {
			return errors.New("invalid or unauthenticated non-loopback listen host")
		}
		host = configured
	}
		...
		go func() {
			...
			for {
				...
				case <-ticker.C:
					...
					if err := s.ingestion.CleanupIngest(cleanup); err != nil {
						log.Print("[server] ingest ledger cleanup incomplete")
					}
					stop()
				}
			}
...
func queryMetricsPage(ctx context.Context, db storage.Store, q storage.MetricQuery) ([]storage.MetricRow, error) {
	if err := storage.RequireTenant(q.TenantID); err != nil {
		return nil, err
	}
	if q.Ctx == nil {
		q.Ctx = ctx
	...
	return querier.QueryMetricsQ(q)
}
...
func (s *Server) handleMetrics(w http.ResponseWriter, r *http.Request) {
	...
	if !ok || !s.enforceRate(w, r, tenant) {
		return
	}
	...
	defer cancel()
	rows, err := queryMetricsPage(ctx, s.db, storage.MetricQuery{Ctx: ctx, Since: time.Now().Add(-parseRange(rangeParam)), TenantID: tenant, ServerID: r.URL.Query().Get("server_id"), Limit: page.Limit, AfterUnix: page.AfterUnix, AfterID: page.AfterID})
	if err != nil {
		writeJSONError(w, 503, "metric query unavailable")
		return
	}
	...
	next, complete := "", true
	if len(rows) > 0 {
		next, complete = nextReadCursor(rows[len(rows)-1].Timestamp.Unix(), rows[len(rows)-1].ID, len(rows), page.Limit)
	} else {
		complete = true
	}
	s.writeCORS(w, r)
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("X-Result-Limit", strconv.Itoa(page.Limit))
	w.Header().Set("X-Result-Complete", strconv.FormatBool(complete))
	json.NewEncoder(w).Encode(metricsResponse{Range: rangeParam, Count: len(data), Complete: complete, NextCursor: next, Data: data})
}
...
func (s *Server) handleProcesses(w http.ResponseWriter, r *http.Request) {
	...
	defer cancel()
	q, ok := s.db.(interface {
		QueryProcessesQ(storage.MetricQuery) ([]storage.ProcessRow, error)
	})
	if !ok {
		writeJSONError(w, 503, "bounded process query unavailable")
		return
	}
	rows, err := q.QueryProcessesQ(storage.MetricQuery{Ctx: ctx, Since: time.Now().Add(-parseRange(rangeParam)), Until: time.Now(), TenantID: tenant, ServerID: r.URL.Query().Get("server_id"), Limit: page.Limit, AfterUnix: page.AfterUnix, AfterID: page.AfterID})
	if err != nil {
		writeJSONError(w, 503, "process query unavailable")
		return
	}
	data := make([]processDataPoint, 0, len(rows))
	for _, row := range rows {
		data = append(data, processDataPoint{Timestamp: row.Timestamp.Unix(), ServerID: row.ServerID, PID: row.PID, Name: row.Name, CPUPct: row.CPUPct, MemMB: row.MemMB})
	}
	next, complete := "", true
	if len(rows) > 0 {
		next, complete = nextReadCursor(rows[len(rows)-1].Timestamp.Unix(), rows[len(rows)-1].ID, len(rows), page.Limit)
	...
	w.Header().Set("X-Result-Limit", strconv.Itoa(page.Limit))
	w.Header().Set("X-Result-Complete", strconv.FormatBool(complete))
	json.NewEncoder(w).Encode(processResponse{Range: rangeParam, Count: len(data), Complete: complete, NextCursor: next, Data: data})
}
...
func (s *Server) healthSnapshot() (bool, error, time.Duration, bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if !s.healthCheckedAt.IsZero() && time.Since(s.healthCheckedAt) < 10*time.Second {
		return s.healthHealthy, s.healthDBErr, 0, false
	}
	var err error
	if s.hubMode && s.ingestionErr != nil {
		err = s.ingestionErr
	} else if p, ok := s.db.(pinger); ok {
		ctx, cancel := context.WithTimeout(context.Background(), healthPingLimit)
		err = p.Ping(ctx)
		cancel()
	} else {
		err = errors.New("storage readiness check unavailable")
	}
...
func (s *Server) handleReady(w http.ResponseWriter, r *http.Request) {
	...
	if healthy, _, _, _ := s.healthSnapshot(); !healthy {
...
func (s *Server) handleExportCSV(w http.ResponseWriter, r *http.Request) {
	...
	if err := storage.RequireTenant(tenant); err != nil {
		writeJSONError(w, 400, "tenant required")
		return
	}
	page, err := parseReadPage(r)
	if err != nil {
	...
	defer cancel()
	view := &exportView{Store: s.db, ctx: ctx, server: r.URL.Query().Get("server_id"), limit: page.Limit, afterUnix: page.AfterUnix, afterID: page.AfterID}
	var buf strings.Builder
	n, err := export.WriteCSV(&buf, view, time.Now().Add(-parseRange(r.URL.Query().Get("range"))), tenant)
	if err != nil {
		log.Print("[server] CSV export incomplete")
		writeJSONError(w, 503, "export unavailable")
		return
	}
	next, complete := nextReadCursor(view.lastUnix, view.lastID, n, page.Limit)
	if n == 0 {
		complete = true
		next = ""
	}
...
func (v *exportView) QueryMetrics(since time.Time, tenant string) ([]storage.MetricRow, error) {
	limit := v.limit
	if limit < 1 {
		limit = storage.DefaultQueryLimit
	}
	rows, err := queryMetricsPage(v.ctx, v.Store, storage.MetricQuery{Ctx: v.ctx, Since: since, TenantID: tenant, ServerID: v.server, Limit: limit, AfterUnix: v.afterUnix, AfterID: v.afterID})
	if err != nil {
		return nil, err
	}
	if len(rows) > 0 {
		v.lastUnix = rows[len(rows)-1].Timestamp.Unix()
		v.lastID = rows[len(rows)-1].ID
	}
	return rows, nil
}
...
func (s *Server) handleServers(w http.ResponseWriter, r *http.Request) {
	...
	defer cancel()
	q, ok := s.db.(interface {
		QueryServersPage(context.Context, string, int, string) ([]string, error)
	})
	if !ok {
		writeJSONError(w, 503, "bounded server query unavailable")
		return
	}
	servers, err := q.QueryServersPage(ctx, tenant, page.Limit, page.After)
	if err != nil {
		writeJSONError(w, 503, "server query unavailable")
		return
	}
	if servers == nil {
		servers = []string{}
	}
	next, complete := "", len(servers) < page.Limit
	if !complete && len(servers) > 0 {
		next = servers[len(servers)-1]
	}
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("X-Result-Limit", strconv.Itoa(page.Limit))
	w.Header().Set("X-Result-Complete", strconv.FormatBool(complete))
	...
	json.NewEncoder(w).Encode(map[string]interface{}{"servers": servers, "limit": page.Limit, "complete": complete, "next_cursor": next})
}
...
func (s *Server) receiveBatch(w http.ResponseWriter, r *http.Request, legacy bool) {
	...
	if principal.Kind != storage.KindAgent || principal.AgentID == "" {
		writeJSONError(w, 403, "a bound machine credential is required")
		return
	}
	if !s.limiter.allow("agent:" + principal.TenantID + ":" + principal.AgentID) {
		w.Header().Set("Retry-After", "1")
		writeJSONError(w, 429, "agent request rate exceeded")
		return
	}
	if s.ingestion == nil || s.ingestionErr != nil {
		writeJSONError(w, 503, "atomic ingestion unavailable")
		return
	}
	if s.retentionStatus != nil {
		if pressure, _ := s.retentionStatus()["disk_pressure"].(bool); pressure {
			w.Header().Set("Retry-After", "60")
			writeJSONError(w, 503, "storage disk budget exhausted")
			return
		}
	}
	body, err := io.ReadAll(http.MaxBytesReader(w, r.Body, maxIngestBody))
	if err != nil {
		writeJSONError(w, 413, "body exceeds limit")
		return
	}
	...
	if legacy {
		...
		if err != nil {
			writeJSONError(w, 400, "invalid legacy event")
			return
		}
	...
	} else if err := storage.DecodeIngest(body, &batch); err != nil {
		writeJSONError(w, 400, "invalid batch JSON")
		return
	}
	if err := storage.NormalizeIngest(&batch, principal.TenantID, principal.AgentID, time.Now()); err != nil {
		writeJSONError(w, 400, "invalid event identity, timestamp or value")
		return
	}
	ctx, cancel := context.WithTimeout(r.Context(), 10*time.Second)
	defer cancel()
	outcomes, err := s.ingestion.AcceptIngest(ctx, principal.TenantID, principal.AgentID, batch, s.ingestPolicy)
	if errors.Is(err, storage.ErrEventConflict) {
		writeJSONError(w, 409, "event content conflicts with its accepted identity")
		return
	}
	...
	if err != nil {
		writeJSONError(w, 503, "ingest transaction did not complete")
		return
	}
	w.Header().Set("Content-Type", "application/json")
