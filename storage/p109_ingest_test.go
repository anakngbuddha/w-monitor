...
func ingestFixture(t *testing.T) (*DB, IngestBatch) {
	...
	if err := db.InitializeIngest(context.Background()); err != nil {
		t.Fatal(err)
	}
...
func TestIngestReplayConflictAndAcceptedBudget(t *testing.T) {
	...
	if err != nil || out[0].Status != "accepted" {
		t.Fatalf("first: %v %v", out, err)
	}
	out, err = db.AcceptIngest(ctx, "tenant-a", "agent-a", batch, policy)
	if err != nil || out[0].Status != "duplicate" {
		t.Fatalf("replay: %v %v", out, err)
	}
	batch.Events[0].Metric.CPUPct = 21
	if _, err := db.AcceptIngest(ctx, "tenant-a", "agent-a", batch, policy); !errors.Is(err, ErrEventConflict) {
		t.Fatalf("changed replay: %v", err)
	}
	batch.Events[0].EventID, batch.Events[0].Sequence = "event-2", 2
	if _, err := db.AcceptIngest(ctx, "tenant-a", "agent-a", batch, policy); !errors.Is(err, ErrAcceptedBudget) {
		t.Fatalf("budget: %v", err)
	}
	n, err := db.CountMetrics()
	if err != nil || n != 1 {
		t.Fatalf("metric count=%d err=%v", n, err)
	}
}
...
func TestIngestInvalidBatchMakesNoWrites(t *testing.T) {
	...
	if _, err := db.AcceptIngest(context.Background(), "tenant-a", "agent-a", batch, DefaultIngestPolicy()); err == nil {
...
func TestIngestStrictJSON(t *testing.T) {
	for _, raw := range []string{`{"schema_version":"x","schema_version":"y"}`, `{} {}`, `{"unknown":1}`} {
		...
		if err := DecodeIngest([]byte(raw), &batch); err == nil {
