...
func TestPhase1PostgresFixture(t *testing.T) {
	if os.Getenv("WMONITOR_PHASE1_PG_FIXTURE") != "1" {
		t.Skip("NOT RUN: disposable PostgreSQL fixture not enabled")
	}
	...
	if err != nil {
		t.Fatal("disposable PostgreSQL fixture unavailable")
	}
	...
	defer first.Close()
	if err := first.InitializeIngest(ctx); err != nil {
		t.Fatal(err)
	}
	...
	defer second.Close()
	if err := second.InitializeIngest(ctx); err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || len(out) != 1 || out[0].Status != "accepted" {
		t.Fatalf("initial acceptance: %v %v", out, err)
	}
	out, err = second.AcceptIngest(ctx, "tenant-a", "same-agent", batch, policy)
	if err != nil || len(out) != 1 || out[0].Status != "duplicate" {
		t.Fatalf("replica lost-ACK replay: %v %v", out, err)
	}
	batch.Events[0].Metric.CPUPct = 26
	if _, err := second.AcceptIngest(ctx, "tenant-a", "same-agent", batch, policy); !errors.Is(err, ErrEventConflict) {
		t.Fatalf("changed replay: %v", err)
	}
	batch.Events[0].EventID, batch.Events[0].Sequence = "event-2", 2
	if _, err := second.AcceptIngest(ctx, "tenant-a", "same-agent", batch, policy); !errors.Is(err, ErrAcceptedBudget) {
	...
	if err != nil || len(rows) != 1 || rows[0].CPUPct != 25 {
		t.Fatal("replay or rejected transaction changed evidence")
	}
	foreign, err := first.QueryMetricsQ(MetricQuery{Ctx: ctx, TenantID: "tenant-b", Since: time.Now().Add(-time.Hour)})
	if err != nil || len(foreign) != 0 {
		t.Fatal("foreign tenant saw evidence")
	}
}
