...
func TestStorageRoundtrip(t *testing.T) {
	...
	// Read back
	metrics, err := db.QueryMetrics(now.Add(-time.Minute), storage.LocalTenantID)
	if err != nil {
	...
	processes, err := db.QueryProcesses(now.Add(-time.Minute), storage.LocalTenantID)
	if err != nil {
	...
	// Test QueryServers
	servers, err := db.QueryServers(storage.LocalTenantID)
	if err != nil {
...
func BenchmarkQueryMetrics(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		rows, err := db.QueryMetrics(since, storage.LocalTenantID)
		if err != nil {
