...
func boundedLimit(limit int) int {
	if limit < 1 || limit > DefaultQueryLimit { return DefaultQueryLimit }
	return limit
}

func processQuery(q MetricQuery) (string, []any, error) {
	if err := RequireTenant(q.TenantID); err != nil { return "", nil, err }
	...
	if !q.Until.IsZero() { args = append(args, q.Until.Unix()); query += fmt.Sprintf(" AND timestamp<=$%d", len(args)) }
	if q.ServerID != "" { args = append(args, q.ServerID); query += fmt.Sprintf(" AND server_id=$%d", len(args)) }
...
func (db *DB) QueryProcessesQ(q MetricQuery) ([]ProcessRow, error) {
	...
	for rows.Next() {
		var p ProcessRow
		var ts int64
		if err := rows.Scan(&p.ID, &ts, &p.TenantID, &p.ServerID, &p.Hostname, &p.PID, &p.Name, &p.CPUPct, &p.MemMB); err != nil { return nil, err }
...
func (db *DB) QueryServersContext(ctx context.Context, tenant string, limit int) ([]string, error) {
	if err := RequireTenant(tenant); err != nil { return nil, err }
	rows, err := db.conn.QueryContext(ctx, `SELECT DISTINCT server_id FROM metrics WHERE tenant_id=$1 AND server_id!='' ORDER BY server_id LIMIT $2`, tenant, boundedLimit(limit))
	if err != nil { return nil, err }
	defer rows.Close()
	var out []string
...
func (pg *PostgresDB) QueryServersContext(ctx context.Context, tenant string, limit int) ([]string, error) {
	if err := RequireTenant(tenant); err != nil { return nil, err }
	rows, err := pg.pool.Query(ctx, `SELECT DISTINCT server_id FROM metrics WHERE tenant_id=$1 AND server_id!='' ORDER BY server_id LIMIT $2`, tenant, boundedLimit(limit))
	if err != nil { return nil, err }
	defer rows.Close()
	var out []string
	for rows.Next() { var id string; if err := rows.Scan(&id); err != nil { return nil, err }; out = append(out, id) }
