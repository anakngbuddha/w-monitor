...
func acknowledgeBatch(w http.ResponseWriter, body []byte) {
	...
	if err := json.Unmarshal(body, &batch); err != nil {
		http.Error(w, "invalid fixture batch", 400)
		return
	}
	out := make([]storage.IngestOutcome, len(batch.Events))
	for i, event := range batch.Events {
		out[i] = storage.IngestOutcome{EventID: event.EventID, Status: "accepted"}
	}
...
func TestAgentIngestMetric(t *testing.T) {
	...
	if err := a.InsertMetric(m); err != nil {
		t.Fatal(err)
	}
	select {
	case got := <-received:
		if got.err != nil || got.payloadType != "/api/v1/ingest/batches" || got.key != key {
			t.Fatalf("incorrect request: %+v", got)
		}
		var batch storage.IngestBatch
		if err := json.Unmarshal(got.body, &batch); err != nil {
			t.Fatal(err)
		}
		if batch.SchemaVersion != storage.IngestSchema || len(batch.Events) != 1 {
			t.Fatal("invalid batch envelope")
		}
		event := batch.Events[0]
		if event.EventID == "" || event.BootID == "" || event.Sequence == 0 || event.Metric == nil || event.Metric.ServerID != m.ServerID || event.Metric.CPUPct != m.CPUPct {
			t.Fatal("event identity or metric changed")
		}
	case <-time.After(3 * time.Second):
		t.Fatal("no metric delivered")
	}
}
...
func TestAgentIngestProcess(t *testing.T) {
	...
	if err := a.InsertProcess(p); err != nil {
		t.Fatal(err)
	}
	select {
	case batch := <-received:
		if len(batch.Events) != 1 || batch.Events[0].Process == nil || batch.Events[0].Process.PID != p.PID || batch.Events[0].Process.Name != p.Name {
			t.Fatal("process batch changed")
		}
	case <-time.After(3 * time.Second):
		t.Fatal("no process delivered")
	}
}
...
func TestAgentUnauthorizedRetainsDurableEvidence(t *testing.T) {
	...
	if err := a.InsertMetric(storage.MetricRow{Timestamp: time.Now(), ServerID: "fixture-agent"}); err != nil {
		t.Fatal("durable enqueue failed:", err)
	}
	select {
	case <-attempted:
	case <-time.After(3 * time.Second):
	...
	if depth := a.SpoolDepth(); depth != 1 {
		t.Fatalf("401 lost evidence: depth=%d", depth)
	}
}
...
func TestAgentExplicitDirectoryPersistsOnlyItsBacklog(t *testing.T) {
	...
	if a.SpoolDepth() != 1 {
		a.Close()
		t.Fatal("durable enqueue missing")
	}
	if err := a.Close(); err != nil {
		t.Fatal(err)
	}
	...
	defer b.Close()
	if b.SpoolDepth() != 1 {
		t.Fatal("backlog did not survive reopen")
	}
...
func TestAgentExplicitDirectoryRejectsFallback(t *testing.T) {
	for _, dir := range []string{"", ".", "relative"} {
		if a, err := agent.NewWithDataDir("https://hub.example.com", "fixture-key", dir); err == nil {
			a.Close()
	...
	if a, err := agent.NewWithDataDir("https://hub.example.com", "fixture-key", file); err == nil {
		a.Close()
		t.Fatal("filesystem error disabled spooling")
	}
}
