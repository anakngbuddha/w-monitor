...
func newAgent(hubURL, apiKey, dataDir string) *Agent {
	...
	if dataDir == "" {
		a.initErr = errors.New("agent: durable spool is required")
		return a
	}
	...
	if err != nil || boot == 0 {
		a.initErr = errors.New("agent: OS boot identity unavailable")
		return a
	}
...
func (a *Agent) BindIdentity(tenant, server string) {
	a.mu.Lock()
	defer a.mu.Unlock()
	if a.spool == nil || tenant == "" || server == "" {
		a.initErr = errors.New("agent: complete spool identity is required")
		return
	}
	if err := a.spool.BindDestination(a.hubURL, tenant, server); err != nil {
		a.initErr = err
		return
	}
	a.bound = true
...
func (a *Agent) StartDrainer(parent context.Context) {
	a.mu.Lock()
	defer a.mu.Unlock()
	if a.closed || a.spool == nil || a.initErr != nil || !a.bound {
		return
	}
	a.drainOnce.Do(func() {
		a.workers.Add(1)
...
func (a *Agent) drainLoop(ctx context.Context) {
	...
	for {
		...
		if failed {
			...
			if errors.As(err, &retry) && retry.retryAfter > wait {
...
func (a *Agent) QueryMetrics(time.Time, string) ([]storage.MetricRow, error) {
	return nil, errors.New("agent: read unsupported")
}
func (a *Agent) QueryProcesses(time.Time, string) ([]storage.ProcessRow, error) {
	return nil, errors.New("agent: read unsupported")
}
...
func (a *Agent) QueryServers(string) ([]string, error) {
	return nil, errors.New("agent: read unsupported")
}
...
func (a *Agent) enqueue(payloadType string, value interface{}) error {
	...
	case storage.MetricRow:
		...
		event.Metric = &row
		server = row.ServerID
	case storage.ProcessRow:
		row.TenantID = ""
		event.Process = &row
		server = row.ServerID
	default:
		a.mu.Unlock()
		return errors.New("agent: unsupported event type")
	}
	if !a.bound {
		...
		if a.spool == nil || server == "" {
			a.mu.Unlock()
			return errors.New("agent: spool and stable server identity required")
		}
		if err := a.spool.BindDestination(a.hubURL, "credential:"+hex.EncodeToString(fingerprint[:]), server); err != nil {
			a.mu.Unlock()
			return err
		}
		a.bound = true
	}
	if a.sequence >= (1<<63)-1 {
		a.mu.Unlock()
		return errors.New("agent: sequence exhausted")
	}
	a.sequence++
...
func (a *Agent) deliverBatch(entries []spoolEntry) error {
	...
	for _, entry := range entries {
		...
		if entry.PayloadType == "event" {
			if err := storage.DecodeIngest(entry.Body, &event); err != nil {
				return errors.New("agent: invalid spooled event")
			}
		} else {
			...
			case "metric":
				...
				if err := storage.DecodeIngest(entry.Body, event.Metric); err != nil {
					return err
				}
				event.Metric.TenantID = ""
				identity = "metric:" + event.Metric.Timestamp.UTC().Format(time.RFC3339Nano)
			case "process":
				event.Process = &storage.ProcessRow{}
				if err := storage.DecodeIngest(entry.Body, event.Process); err != nil {
					return err
				}
				event.Process.TenantID = ""
				identity = fmt.Sprintf("process:%s:%d", event.Process.Timestamp.UTC().Format(time.RFC3339Nano), event.Process.PID)
			default:
				return errors.New("agent: unsupported spooled event")
			}
	...
	if len(body) > storage.MaxBatchBytes {
		return errors.New("agent: batch exceeds byte limit")
	}
...
func (a *Agent) deliver(payloadType string, body []byte) error {
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		if err != nil {
			return errors.New("agent: invalid destination")
		}
		a.mu.Lock()
		...
		if err != nil {
			return &retryableError{err: errors.New("agent: transport failed")}
		}
		responseBody, readErr := io.ReadAll(io.LimitReader(resp.Body, storage.MaxBatchBytes+1))
		resp.Body.Close()
		if readErr != nil || len(responseBody) > storage.MaxBatchBytes {
			return &retryableError{err: errors.New("agent: incomplete or oversized acknowledgement")}
		}
		switch {
		case resp.StatusCode == http.StatusAccepted || resp.StatusCode == http.StatusOK:
			...
			var ack struct {
				...
				Outcomes []storage.IngestOutcome `json:"outcomes"`
			}
			if json.Unmarshal(body, &sent) != nil || storage.DecodeIngest(responseBody, &ack) != nil || ack.Status != "accepted" || len(ack.Outcomes) != len(sent.Events) {
				return &retryableError{err: errors.New("agent: invalid acknowledgement")}
			}
			for i, outcome := range ack.Outcomes {
				if outcome.EventID != sent.Events[i].EventID || (outcome.Status != "accepted" && outcome.Status != "duplicate") {
					return &retryableError{err: errors.New("agent: acknowledgement identity mismatch")}
				}
			}
			return nil
		case resp.StatusCode == http.StatusUnauthorized:
			if attempt == 0 && reauth != nil {
...
func retryAfterDelay(raw string, now time.Time) time.Duration {
	if seconds, err := strconv.ParseInt(raw, 10, 64); err == nil && seconds > 0 && seconds <= int64((1<<63-1)/int64(time.Second)) {
