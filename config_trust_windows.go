...
func checkConfigPermissions(path string, info os.FileInfo) error {
	...
	for _, name := range []string{path, filepath.Dir(path)} {
		...
		if err != nil || item.Mode()&os.ModeSymlink != 0 { return errors.New("unsafe configuration path") }
		sd, err := windows.GetNamedSecurityInfo(name, windows.SE_FILE_OBJECT, windows.DACL_SECURITY_INFORMATION)
		if err != nil { return err }
		sddl := sd.String()
		if !strings.Contains(sddl, "D:P") { return errors.New("config requires a protected DACL") }
		entries := ace.FindAllStringSubmatch(sddl, -1)
		if len(entries) == 0 { return errors.New("empty configuration DACL") }
		for _, entry := range entries {
			parts := strings.Split(entry[1], ";")
			if len(parts) != 6 || parts[0] != "A" || !allowed[parts[5]] { return errors.New("configuration DACL grants an unapproved identity") }
		}
	}
	return nil
}
